...
INSTRUCTION_FILE = 'assets/taskvar_instructions.jsonl'
...
def generate_all_instructions():
    ...
    all_tasks = json.load(open('assets/all_tasks.json'))
...
def main(args):
    taskvar_instrs = load_all_instructions()

    tokenizer, model = load_text_encoder(args.encoder)
    model = model.to(args.device)

    os.makedirs(args.output_file, exist_ok=True)
    lmdb_env = lmdb.open(args.output_file, map_size=int(1024**4))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--generate_all_instructions', action='store_true', default=False)
